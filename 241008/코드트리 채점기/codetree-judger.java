...
public class Main {
    ...
    static Set<Integer>[] isInReadyq = new TreeSet[MAX_D + 1]; // 인덱스 = 도메인의 인덱스
    ...
    static int[] judgingDomain = new int[MAX_N + 1]; // 채점하는 도메인 인덱스
    static int N; // 채점기 갯수
    ...
    public static void play(int t) {
        ...
        Code minCode = new Code(0, INF, Integer.MAX_VALUE);
        for (int i = 1; i <= idx; i++) { // 도메인 인덱스
            ...
            if (!waitingPq[i].isEmpty()) { // i번 도메인에 url이 있다면
                ...
                if (minCode.compareTo(curCode) > 0) { // 우선순위가 더 작거나 문제 번호가 같더라도 들어온 시각이 더 빠르다면
                    minCode = curCode;
                    minDomain = i;
                }
            }
        ...
        if (minDomain > 0) {
            ...
            e[minDomain] = INF; // 채점 종료 시각 할당(아직 언제 끝나는지 모름)
    ...
    public static void end(int t, int jId) {
        ...
        g[domainIdx] = t - s[domainIdx]; // 채점 시간
        e[domainIdx] = s[domainIdx] + 3 * g[domainIdx]; // 종료 시각
    }
}